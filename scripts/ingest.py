...
PANDOC_MARKDOWN_OPTIONS = [
    ...
    '--atx-headers',             # Use # headings (ATX style)
    '--reference-links',         # Collect links at bottom
    '--strip-comments',          # Remove HTML comments from source
]


def run_pandoc(source: Path, output: Path, from_format: str) -> tuple[bool, str]:
...
def ingest(source_path: Path, output_path: Path) -> bool:
    ...
    # Run pandoc
    success, error = run_pandoc(source_path, output_path, from_format)
    if not success:
...
Output:
  Normalized markdown in outputs/[journal]/working.md
    ...
    success = ingest(args.source, args.output)
